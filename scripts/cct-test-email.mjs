...
async function main() {
  const to = readArg('to', process.env.CCT_EMAIL_TO || 'recipient@example.invalid');
  const cc = readArg('cc', process.env.CCT_EMAIL_CC || 'cc@example.invalid');
  ...
  const result = await emailService.sendTestEmail({
    ...
    text: [
      ...
      '',
      `Link para o site: ${process.env.CCT_SITE_URL || 'http://localhost:3000/cct'}`,
    ].join('\n'),
  });
