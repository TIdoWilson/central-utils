...
function createCctEmailService(deps = {}) {
  ...
  const secure = deps.secure === undefined
    ? Number(port) === 465
    : !!deps.secure;
  const user = safeString(deps.user || process.env.CCT_SMTP_USER || 'smtp-user@example.invalid');
  const pass = safeString(deps.pass || process.env.CCT_SMTP_PASS || '');
  const from = safeString(deps.from || process.env.CCT_EMAIL_FROM || user);
  const siteUrl = safeString(deps.siteUrl || process.env.CCT_SITE_URL || 'http://localhost:3000/cct');
  const errorRecipient = safeString(deps.errorRecipient || process.env.CCT_ERROR_RECIPIENT || 'recipient@example.invalid');
