...
module.exports = function createCctRoutes(deps = {}) {
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      return res.status(500).json({ ok: false, error: 'Erro ao listar convencoes.' });
    }
  });
  ...
  router.get('/historico', async (req, res) => {
    try {
      ...
      await auditLog?.(req, 'tool_cct_history', 'error', {
        error: String(error?.message || error),
      });
      return res.status(500).json({
        ok: false,
        error: 'Erro ao carregar o historico.',
      });
    }
  });
  ...
  router.get('/:id/download', async (req, res) => {
    try {
      ...
      const status = Number(error?.statusCode) || 500;
      if (status >= 500) {
        console.error('Erro ao baixar convencao CCT:', error?.message || error);
      }
      await auditLog?.(req, 'tool_cct_download', status === 404 ? 'error' : 'error', {
        conventionId: req.params.id,
        error: String(error?.message || error),
      });
      return res.status(status).json({
  ...
  router.get('/:id', async (req, res) => {
    try {
      ...
      await auditLog?.(req, 'tool_cct_detail', 'ok', {
        conventionId: req.params.id,
        clausulas: detail?.quantidadeClausulas || 0,
      });
      return res.json({ ok: true, item: detail });
    } catch (error) {
      const status = Number(error?.statusCode) || 500;
      if (status >= 500) {
        console.error('Erro ao detalhar convencao CCT:', error?.message || error);
      }
