...
- **Página (rota):** `/dimob`
- **API base:** `/api/dimob`
- **Permissão RBAC:** `tool:dimob` ou `tool:*` (ADMIN acessa)
...
- **Página HTML:** `public/dimob.html`
- **Script JS da ferramenta:** `public/js/dimob.js`
- **Router Node:** `src/routes/tools/dimob.routes.js`
- **Service Node:** `src/services/dimob.service.js`
- **Arquivos Python relacionados:** _não foi identificado arquivo Python específico para este slug_

## 4. Rotas e Endpoints

- **Rota de página:** `/dimob`;
- **Base de API esperada:** `/api/dimob`;
- **Endpoints no router:**
  - `GET /previous-file`
  - `POST /parse-sped`
  - `POST /generate-file`
