...
- **Slug:** `irpf`
- **Grupo:** Geral
- **Página (rota):** _não exposta diretamente como página_ (usa interface dedicada via integração)
- **API base:** `/api/irpf`
- **Permissão RBAC:** `tool:irpf` ou `tool:*` (ADMIN acessa)

Ferramenta IRPF no portal, com fluxo autenticado e controle de acesso por RBAC.
