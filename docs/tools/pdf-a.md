...
- **Página (rota):** `/pdf-a`
- **API base:** `/api/pdfa`
- **Permissão RBAC:** `tool:pdf-a` ou `tool:*` (ADMIN acessa)
...
- **Página HTML:** `public/pdf-a.html`
- **Script JS da ferramenta:** `public/js/pdfa.js` (alias técnico usa `pdfa.js`)
- **Router Node:** `src/routes/tools/pdfa.routes.js`
- **Service Node:** `src/services/pdfa.service.js`
- **Arquivos Python relacionados:** _não foi identificado arquivo Python específico para este slug_

## 4. Rotas e Endpoints

- **Rota de página:** `/pdf-a`;
- **Base de API esperada:** `/api/pdfa`;
- **Endpoints no router:**
  - `POST /convert`
  - `GET /download/:id`
