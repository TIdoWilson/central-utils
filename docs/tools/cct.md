...
- Ao incluir um CNPJ no site, ele entra em `CNPJ.txt`, registra o solicitante em `CNPJ_requisitantes.txt` e interrompe a fila atual para processar primeiro o pedido recem-incluido.
...
- **Novo CNPJ entra na fila, mas nao e pesquisado logo em seguida:** sintoma: o numero aparece nos TXT, mas fica aguardando o termino da rodada completa e a busca do sindicato solicitado nao acontece na hora. Causa provavel: a rota do `/cct` estava usando a fila comum e apenas marcando `rerunRequested`, deixando o pedido preso atras do processamento em andamento. Solucao: o backend agora usa a entrada exclusiva para pedidos do site, interrompe a rodada atual, processa primeiro o CNPJ solicitado e depois restaura a fila automatica.
